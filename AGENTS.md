...
- **Platform prerequisites** are `scripts/setup_platform.sh` — one idempotent entry point called by
  ...
  permissions + vacating the UDC's kernel configfs gadget instances; a UDC held by a process with `/dev/raw-gadget` open is released only when that process exits, which is the one thing no script step can do) and **axcl** (verified only — the library directory and
...
- **The journal is persistent on the board** — a board-side deployment fact, not a repository
  artefact: `Storage=persistent` in `/etc/systemd/journald.conf`, `/var/log/journal` created and
  `systemd-journald` restarted, so a wedged shutdown leaves evidence instead of nothing. Look with
...
| `src/core/proc_util.cpp/.h` | the process gate and the `/proc` diagnostics behind it: `instance_lock_acquire` takes an `flock` on `/run/aimbot.lock` (tmpfs, so a reboot clears it) and, when the lock is already held, prints the holder — pid read from the lock file, command line from `/proc/<pid>/cmdline` (NUL-separated argv joined into one line, empty when it cannot be read) — and returns false, which the caller turns into a non-zero exit **before** any device is touched. The lock lives on the open file description, so a crash or a `kill -9` normally releases it in the kernel — but **not always**: measured, a session killed while stuck in an endpoint ioctl leaves the **lock record** on the inode attributed to the (now zombie) pid while **no live process has the file open** (`/proc/locks` names it, `/proc/*/fd` does not), and the remnant path rebuilds the lock file exactly in that case — its "the owner is gone" test therefore counts a **zombie** as gone, because the file's own content is a stale pid that the kernel may already have reused for an unrelated task; that pid-reuse misreading of "the previous instance is still running", a pidfile check has too. `proc_fd_holders` scans `/proc/*/fd` for the processes holding a device node, comparing the node's identity (char device `st_rdev`) rather than a path string — a symlinked or bind-mounted spelling of the same node still counts — and returns them sorted by pid with the caller excluded (it holds its own descriptor by construction). Consumed by `main.cpp` (the instance gate) and `io/usbraw.cpp` (naming the UDC's holders on `EBUSY`) |
...
| `webui/` | the browser control panel (FastAPI + vanilla JS): profile discovery from `scripts/game/*.sh`, model discovery from `engine/**/*.axmodel`, receiver discovery from `/sys/class/video4linux` with the live signal state (`v4l2-ctl --all`), the start sequence (`scripts/setup_platform.sh` → `bin/aimbot` with the full parameter set, `-S` pointing at the profile script), hot-param push, the calibration button, compile as an async task and the log stream. Its script write-back carries the same concurrency discipline as the firmware's (writer-identifying temp name + a re-read compare before the rename) and is handed a **patch** rather than a pre-merged snapshot, so its own read-modify-write — read, publish the named keys, rename — sits inside one in-process critical section (the panel is single-process by contract); its per-session FPS history is tracked with a session epoch so an already-open page follows a restart without a reload. `webui/README.md` is the operator manual |
...
reference state machine's dispatch rules; the round starts when the F0 challenge arrives, and the acceptance
condition is the protocol's own — the challenge is 64 bytes and the auth port is idle (an F0 arriving outside
idle is silently dropped). Idle
...
never calibrated (the per-game feel is the four manual `spd` ratios). The engine's three readings produce that
`L` directly — the **physical** loop delay — and the write-back and the running state use that one number
(see "Triggers, feedback, write-back" below). The inference leg the round skips is measured and printed as a
diagnostic only: the law's anchor is the frame-delivery instant, so the post-anchor interval is already
carried by `age`, and adding it to `L` would count the same interval twice.
...
reading, which is what a truncated tail would corrupt; the derivation's 7 frames are the loop's own
end-to-end budget, and the written value is exactly that calibrated reading, so the CLI's `-l` clamp,
`[L_MIN, L_MAX]`, is applied to the same quantity the gate is.

**Gates** (each with its source; the tail-vs-edge difference and every family's median/MAD are printed — as are σ's own pool (its range, zero count, response median and static-block median) and, when σ is what failed, every excitation segment's sample count, signed displacement sum and per-frame magnitude, the two lines that make a failed round diagnosable without a rerun —
...
writes is the calibrated physical loop delay** — what this engine's three readings measure (they are all
anchored on frame timestamps), which is also the value the running `l_est` takes. The inference leg
(`H2D + exec + D2H + decode` — the segment the `[AI FPS]` line already reports) is deliberately *not* added:
the law's timestamp anchor is the frame-delivery instant (`wait_frame` returning), so the pipeline work after
that anchor is already carried by the law's `age` and `gap_scale`, and adding it to `L` would count the same
interval twice. Measured on the 640 model, whose inference leg is 6.39 ms: a belief of 56.4 ms instead of 50
puts `arena.eval ff_pi_acc 56.4`'s matched composite at 129.11 against the shipped 114.66 — the very gate that
forbids a rise. The two legs the round **does** pay — waiting for a frame's payload and the RGA crop — are
likewise *not* added: the calibration round waits for the same frames and crops the same 640 window itself, so
those are shared legs already inside every reading. The round's log carries the physical `L` and, beside it,
the averaged inference leg with the frame count it was averaged over — labelled a diagnostic, so no reader
mistakes it for part of `L`; a round whose process has not run an inference frame yet prints no average, and
nothing is ever invented to fill that slot.
The leg is a measured, load-dependent quantity rather than a constant, and the diagnostic's running mean is
the right summary of that: on this board with the shipped 320-px 4-class model the `[AI FPS]` line reads
2.39 ms mean over 7187 frames in a quiet 60 s window and 3.66 ms mean over 6767 frames while another NPU
consumer shared the card (H2D 0.76 exec 1.25 D2H 0.28 解码 0.05 against 1.42 / 1.27 / 0.87 / 0.05 ms) — the
two transfer segments, not the NPU's own execution, are what the contention moves.
`-a n` (pure pass-through) makes
...
radius. The **deployment source is 2560×1440**, i.e. R = 1440/1080 = **4/3** against that reference. Five
quantities fall under the rule, in two directions — the first three are what the repository ships, the last
two are the rule's reading of two compile-time constants the code does not rescale (see the note below):
...
| `TRACK_JUMP_GATE` (px) | filter-reset gate on the α-β innovation | **× R** — **未实现, 待定** (see below) | the quantity it thresholds is the detection's screen displacement from the filter's own prediction, i.e. the same kind of length as `FOV_RADIUS`; the arena pins both ends of the comparison at the reference scale (its relock scenario jumps the target ≈143 px, from (80,0) to (−60,30), and `JUMP_GATE` sits at 100 px below it), and at 100 px the gate is two orders of magnitude above the detector's own pixel jitter (arena's 0.5 px detection noise), so it is set against the displacement scale rather than the jitter scale |
| `FF_I_GATE` (px) | settled-region gate / I distance-decay scale | **× R** — **未实现, 待定** (see below) | with the gate closed by a large error the integrator's ramp rate tends to `Ki·FF_I_GATE` (px/ms per ms), a velocity-per-time in this loop, so it scales with the pixels per degree exactly as `MAX_SPEED` does |
...
**未验证 / 待定 — the last two rows are the rule's reading, not the firmware's behaviour.** `TRACK_JUMP_GATE`
and `FF_I_GATE` are pixel-denominated lengths in the same window domain as the law's error signal, and both
values were selected in the arena, which is written at the reference scale (`arena/laws/ff_pi_acc.py` carries
`JUMP_GATE = 100` and `i_gate = 8.0`, the latter documented there as a physical distance in px;
`arena/integrate.py`'s relock jump is the ≈143 px step above). The rule therefore reaches them — but unlike
`MAX_SPEED` and `FOV_RADIUS`, which are operator values with a CLI flag, a hot-param key and a launcher VAR,
these two are compile-time header constants with no entry point, so **the code does not rescale them**: at the
shipped 1440p work point they sit 4/3 tighter in angle than at the reference (the filter resets on a 4/3
smaller angular jump; the I term's far-regime ramp is 4/3 below the reference's, i.e. 25 % less I authority
during large errors). Recorded rather than patched: the direction above is arithmetic from the scaling rule,
not a board measurement, and no measurement separates a rescaled gate from the shipped one (**未验证** —
a resolution-scaled gate would have to be re-validated against the delay band and the FPS suites before it
could ship, exactly like any other law constant).
...
| `TRACK_JUMP_GATE` / `FF_I_GATE` | 100 px / 8 px | unchanged (**未实现** — the rule reaches them but the code has no entry point for it; see the note above) |
...
| `FF_I_GATE` | 8.0 | Settled-region gate / I distance-decay scale / withdrawal-weight scale (px) | Tracking trail → raise; flick overshoot → lower. **The rule reaches it (× R) but the code does not rescale it — open item, see the resolution rule's note** |
...
| `TRACK_JUMP_GATE` | 100 px | Innovation above this resets the filter | Raise only if a real re-lock is being rejected mid-track. **The rule reaches it (× R) but the code does not rescale it — open item, see the resolution rule's note** |
...
- **未验证: a successful round's write-back line has not been seen on this board.** The board's
  signal source is a static screen, so every round here ends in the honest "no motion" failure (σ = 0.000
  exactly, script byte-identical) — the *inference leg* is measured and printed by the `[AI FPS]` line as a
  diagnostic, and the write-back contract (the value that lands is the caller's argument, which the capture
  side takes straight from the engine's physical `l_est`) is pinned by `build/calib_test`, but the round's
  own success line (`[标定] L=<physical>` with the inference average beside it) has
