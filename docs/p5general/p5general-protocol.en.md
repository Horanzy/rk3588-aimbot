...
The following facts come from measurements on a real P5 General dongle with a PS5 as host (the dongle's behaviour is a property of the device, not of the board driving it); they answer part of section 10 directly. Reproduction path: `sudo python3 scripts/test/p5g_dongle_probe.py` (dongle side) plus launching the backend with `-M p5g` while watching the console (PS5 side). **The host side has been accepted by a real console once, but only up to the auth handshake**: with `-M p5g` on the deployment board (RK3588) the PS5 completed enumeration (`/sys/class/udc/*/state` reads `configured`) and read the device definition (the log carries `ℹ [P5G] PS5 读取设备定义 (0x03, wLength 48)`) — which is exactly what the 48-byte static reply and the section 2/3 descriptors exist for; the auth handshake itself was not observed to start, and the reference firmware's own rule explains why (it begins only after the first physical input, a PS-button press), so "the pad is assigned and input reaches the host" remains unverified. The dongle-side facts stand as recorded.
