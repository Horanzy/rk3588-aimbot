...
- 除首页/静态资源/登录端点外所有 API 要求 token（HTTP 头/查询串与 WS 的 `?token=` 走同一条
  判据）；登录/设密失败有固定 1s 延迟 —— 两个端点是 async 的, 延迟期间事件循环只挂起自己,
  不占线程池, PBKDF2 那一段也跑在线程池里: 未认证突发打不垮面板的响应性。
...
- **脚本 = 唯一事实源**: profile 参数就是 game 脚本头部的 `VAR=value` 块, 每次扫描现场解析,
  ...
  - 只改目标变量的值 —— 行内注释、引号风格、其余每一行逐字保留, 执行位不变;
    ...
    行尾统一成 LF（触碰行由值重建, 保留 CRLF 会得到两种行尾混排的脚本）;
  - **文本类值不收在启动脚本里有特殊含义的字符**（引号、`$`、反引号、反斜杠、`#`、控制字符、
    `;|&<>(){}[]*?!~`）: 值落在行内原有的双引号里, 而引号、`$(…)`、反引号都能让值越出引号 ——
    一次网页保存就成了往用户的启动脚本里落笔任意 shell 片段, 换行则直接在脚本里长出新的一行。
    判据是排除而不是白名单: 中文目录名 (`dataset/<游戏名>/`) 与带空格的目录是合法部署形态, 照收。
    被判非法的值不写（回退默认或保持现值）, 并在保存响应的 `rejected` 里点名（键/值/
    原因）, 页面弹出提示 —— 提交了却没落笔的值不会得到一句"已保存"的假话;
  - 手上没有值的项 (未选模型等) 不写 —— 绝不把用户的模型路径抹成空串; 清空一个路径/输出目录
    同样是"不写", 但它是**提交了的**值, 所以落在同一份 `rejected` 里, 页面说得出是哪一项;
...
- **【启动】= 保存当前设置 → 清掉所有在跑实例（`/proc/*/exe` 精确匹配 `<根>/bin/aimbot`，
  两侧都先 `realpath` 归一 —— 符号链接的部署根同样命中；含 SSH 手跑的；SIGTERM 后等**实例锁
  `/run/aimbot.lock` 释放** —— 锁由内核在持锁进程彻底退出时才释放, 僵尸态下 `/proc` 项仍在
  而资源早已交出; 锁判不了时退回 `/proc` 判据 —— 5s 后锁仍被占才 SIGKILL）→
  `scripts/setup_platform.sh` → aimbot**。
  清场不看面板自己认不认得这个实例（`/proc` 扫描没命中就立即返回, 命中了就必须先收掉）——
  面板起来**之后**才在别处跑起来的实例同样先被收掉, 否则新进程撞上实例锁、`rc=1` 退出, 页面
  只会报一句"异常退出"。这段等待（最长 5s）在实例状态锁之外, 面板的 HTTP 与 WebSocket 不被它
  拖住（同一个理由: WS 心跳里的重扫与实例快照都跑在线程池里）。
...
- **孤儿认领**：WebUI 自身重启时扫描已存活的 aimbot 并认领为 running（认领实例无日志，
  可停止/接管）；状态停在 `stopped`/`exited` 时还会定期（~5s，WS 心跳那一拍）回看 `/proc` ——
  面板起来**之后**才跑起来的实例同样会被认领，绝不把存活实例显示成“已停止”诱导双开，
  【启动】也不会在没清场的情况下撞实例锁。
...
6. 【停止】→ 状态“已退出”；SSH 手动跑 game 脚本 → WebUI 显示“认领”而非“已停止”
   （认领实例「截图 本次」显示 —，「截图 文件夹」照常计数）；**面板先起来、SSH 后跑起来**的实例
   同样在 ~5s 内变成“认领”（状态停在 stopped/exited 时心跳那一拍回看 /proc）；
...
12. 保存写回脚本: 网页改 Y_OFFSET/采集开关 →【保存】→ `cat` 脚本对应行已变且注释原样;
    ...
    把「鼠标匹配子串」填成 `a"; touch /tmp/x; #` →【保存】→ 页面弹出「未写入脚本的项」,
    `MOUSE_KEYWORD` 那一行逐字未动（值里的引号/`;`/`#` 一律不收, 写不进去也不假装写了）;
    另存一个把「输出目录」清空的补丁 → 同样点名上报, 脚本里 `OUT_DIR` 保持现值。
