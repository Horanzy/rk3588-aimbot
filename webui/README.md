...
- **脚本 = 唯一事实源**: profile 参数就是 game 脚本头部的 `VAR=value` 块, 每次扫描现场解析,
  ...
  - **并发**: 脚本有两个写者 —— 本面板的保存与固件经 `-S` 的标定回写, 二者都是整文件读-改-写。
    ...
    共用同一个临时文件、两份内容互相穿插; ②**进程内锁**把两个并发保存请求严格串行 (服务单进程,
    同步端点由线程池并发服务) —— 实测 30 轮同时写同一 profile: 双方内容都在 30/30、无残留临时
    文件; ③跨进程那侧锁管不到 (固件的回写是另一个进程), 靠重命名前的重读比对: 写临时文件之后、
    重命名之前再读一次原文件, 内容变了就按新内容重来 (最多 3 次 = 本写者 + 对手方一次 + 对手方
    自身的一次重试), 用尽仍冲突则**拒写并报错** (页面提示重新保存) —— 不静默丢更新。固件那侧
    同一套比对 (它写的是这一轮的标定结论, 用尽仍落笔)。
    **残留窗口** (乐观比对法固有, 关不掉, 两侧同一格) 只剩"最后一次读原文件"与"重命名"这两步之间
    —— 比对因此放在临时文件写好之后 (否则那一段文件写入会替对方把窗口撑到毫秒级), 但这一步仍不是
    零: 实测 20 轮强制两个进程同时写同一 profile, 16 轮双方都落笔, 4 轮一方的改动被对方的重命名
    覆盖, 40 次调用里 3 次重试用尽报错。实践中它要求"面板保存"与"标定回写"落在同一毫秒级窗口里
    (标定一轮只回写一次), 而落在窗口里的结果要么是明说的失败、要么只丢一行。
