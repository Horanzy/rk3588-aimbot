...
- 参数面按组落在脚本 VAR 上（每组一个 VAR，`app/discover.py` 的 `SCRIPT_VARS` 是唯一映射表）：
  ...
  - **延迟与拉枪倍率 — hid / pad / p5g**（三组，每组 5 个值，只有当前输出模式那一组可见）：
    `HID_L_EST` / `PAD_L_EST` / `P5G_L_EST`（**完整环路延迟** ms = 标定量到的物理延迟 + 推理段
    均值 —— 标定整轮跳过推理，那一腿由固件在正常运行时逐帧累计，取帧/RGA 那两腿已量在里面
    故不再加；标定回写目标，也可手填；固件带宽 `wn` 由它导出，所以它是唯一进运行态的标定量）
...
- 标定值（环路延迟）**永远以脚本为权威**：每次启动现场解析；`-S` 指向 profile 脚本本身，
  ...
  标定期间固件只做测量，结论看日志：成功 = 点头 + 只回写本输出模式的延迟格，值为**完整环路
  延迟**（物理 L + 推理段均值；轮次日志把三个数都打出来，冷启动没有均值时只写物理值并说明），
