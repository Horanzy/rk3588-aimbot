"""运维任务: compile 异步执行 (慢, 绝不阻塞页面), 带日志流与历史。

模型转换不在本仓库里 —— .axmodel 由外部工具链转换后直接放进 engine/, 面板只负责编译
固件与列出模型。同一时刻只允许一个运维任务 (互斥); compile 在实例运行中时生成"失败任务"
说明原因 (运行中的二进制无法被覆盖, ETXTBSY), 而不是默默开始然后炸在链接阶段。
"""
import json
import shlex
import subprocess
