...
const SLOT_SPD = [
  { a: "spd_x", label: "速度倍率 X — 腰射", hot: "spdx",
    info: "拉枪速度倍率, 与有效灵敏度成反比 (有效灵敏度 = 基线/(倍率/100)): 调大 = 同样的期望屏幕速度发出更多 counts / 更大的摇杆偏转 = 屏幕跟得更快。整数步进 (105 / 109), 100 = 基线; 有意义的带是 5..2000, 越界会被固件夹取。三套槽的默认值都是 75 而不是 100: 基线这把尺按 1080p 参考分辨率定, 而部署源是 2560×1440 —— 游戏的**真实**灵敏度是参考的 4/3 倍, 倍率与它成反比 (见 AGENTS.md 的分辨率规则)。本槽的基线见组标题。" },
...
const PARAM_DEFS = [
  ...
  { group: "瞄准", key: "max_speed", label: "速度上限", type: "num", min: 100, max: 20000, step: 50, unit: "px/s", hot: true,
    info: "输出指令的速度钳制。调大: 甩枪更快、远距离转移更利落, 失配时抖动/过冲风险上升; 调小: 稳, 但跟不动快速目标。默认 2667 px/s = 1080p 参考下的 2000 px/s 按部署源 2560×1440 (R = 4/3) 换算 —— 像素量与分辨率绑定, 换源分辨率要按同一条规则再换算 (见 AGENTS.md 的分辨率规则)。🔥 热参数, 保存即生效。" },
  ...
  { group: "瞄准", key: "fov", label: "FOV 半径", type: "num", min: 10, max: 1000, step: 5, unit: "px", hot: true,
    info: "FOV 同时是目标筛选圈与积分器启动边界 (一值两用)。几何关系: 规范窗口是采集帧中心 1:1 裁剪出的 640×640, 模型输入更小时看到的是它再中心 1:1 裁一次, 所以模型像素与屏幕像素 1:1, 检测范围为以准星为中心 ±320px (对角约 452px), 调到 452 以上没有额外效果。默认 200 px = 1080p 参考下的 150 px 按部署源 2560×1440 (R = 4/3) 换算: 同一个**角度**上的交战圈, 换源分辨率要按同一条规则再换算 (见 AGENTS.md 的分辨率规则)。调大: 更远/更偏的目标进入筛选圈, 多目标抢锁风险上升; 调小: 只锁准星附近。🔥 热参数, 保存即生效。" },
  // 三套输出各占一组 5 个值 (延迟 1 + 拉枪倍率 4): 由 MODE_SLOT 生成 —— 切换【输出模式】
  // 显示/编辑的就是本槽那一组, 保存也只提交本槽 (见 savePatch)
  ...modeSlotRows(),
  { group: "瞄准", key: "class_id", label: "目标类别 ID", type: "num", min: -1, max: 255, step: 1, hot: false,
    info: "锁定哪个检测类别 (依模型标签, 如 0=头, 1=身)。-1 = 不筛类别 (固件不钳制 -c, 负数即不筛): 面板照原样传给固件, 于是面板启动与直接跑脚本是同一件事。❄ 冷参数, 下次启动生效。" },
