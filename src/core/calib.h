...
//   默认位上) —— 脚本"少写一行也能起"的承诺不因为一次标定而失效。
//  脚本的另一个写者是面板的保存 (webui/app/discover.py), 两者都是整文件读-改-写, 所以
//   两件事必须成立: (a) 临时名各带各的写者身份 —— 共用一个 ".tmp" 会让两份内容在重命名前
//   互相穿插 (本侧由唯一线程驱动回写, pid 即身份; 面板的同步端点由线程池并发服务, 它的
//   临时名再多带一个线程标识); (b) 写临时文件之后、重命名之前重读一次原文件, 内容变了就
//   换新底重来 —— 否则对方在本回写读之后写下的内容会被这次重命名整段抹掉。重落 = 同一份
//   改动 (本回写只改自己那一行) 在新内容上再走一遍, 因此不会压掉对方的改动; 比对紧贴重命名
//   (临时写入在它之前), 残留窗口只剩这两步之间。
//  PERSIST_ATTEMPTS: 面板那侧把自己的写者用进程内锁串行 (它单进程), 所以本回写面对的对手方
//   就是"一次面板保存": 每次冲突都让本回写把自己那一行叠到对方刚写下的新底稿上, 而对方自己
//   也可能正在重试。3 = 本回写 + 对手方一次 + 对手方自身的一次重试。用尽仍落笔 (标定结论是
//   这一轮的交付物, 不因为持续竞争而不写), 那时只剩乐观比对固有的残留窗口 (最后一次读原文件
//   到重命名之间 —— 面板那侧的强制竞争实测该窗口不为零, 见 webui/README.md)。
const int PERSIST_ATTEMPTS = 3;
