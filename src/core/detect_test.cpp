...
//        "属性在后且恰 6 个"这一物理结构判定; 端到端提案表的候选配额撞上 G 或 3G 时
//        几何规则胜出 (同一个布局在几何缺席时被判成提案表 —— 两支的差别就在这里)
...
//        类分数过 sigmoid; 步长未知 / 分布长度不成 4·2^k 时整块跳过 (不猜分布长度)
...
int main() {
    ...
    // ---------------- [3] 口径判定 ----------------
    {
        ...
        // 端到端提案表的候选配额是导出时的选择, **可能恰好撞上** G 或 3G: 那时几何规则
        //   胜出 —— 判据里既不含那个配额也不含"属性恰 6 个", 只看候选数与网格数的物理
        //   关系 (见 detect.cpp 的 head_kind)。下面的类数刻意取成"退路口径会给出另一支",
        //   于是断言只能由几何那一支满足。
        OutputLayout lc;
        parseOutputLayout(dims3(8400, 6), lc, 640);      // 属性 6 / 候选 8400 = G(640)
        CHECK(head_kind(lc, 1, 640) == HeadKind::kYoloV8,
              "[1,8400,6] @640: 候选数 = G → 几何判 v8-11 (6 属性这个形状让位给几何)");
        CHECK(head_kind(lc, 1, 0) == HeadKind::kEnd2End,
              "同一布局几何缺席 (输入边长未知) 时它按'属性在后且恰 6 个'判成端到端提案表 —— "
              "两支的差别就是几何优先这一条");
        parseOutputLayout(dims3(10647, 6), lc, 416);     // 候选 10647 = 3G(416)
        CHECK(head_kind(lc, 2, 416) == HeadKind::kYoloV5,
              "[1,10647,6] @416: 候选数 = 3G → 几何判 v5 (同上, 撞车时几何胜出)");
        CHECK(head_kind(lc, 2, 0) == HeadKind::kEnd2End,
              "同一布局几何缺席时仍按 6 属性判成端到端提案表");
        parseOutputLayout(dims3(300, 6), lc, 640);
        CHECK(head_kind(lc, 2, 640) == HeadKind::kEnd2End,
              "候选数 300 与 G、3G 都不等时几何判不了 → 端到端 (属性在后且恰 6 个)");
    }
    ...
    // ---------------- [5] DFL 头 ----------------
    {
        ...
        // 分布长度判据 = 4·reg_max ∈ {32,64,128,…} (2 的幂且 ≥32): 只查"能被 4 整除"时
        //   类数错一个仍落在整除带上 (attrs 144 − 类数 124 = 20 = 4·5), 解码照常进行而每条
        //   边的分布长度是错的 → 语法合法、数值错的框。判据把这一带堵死。
        CHECK(dfl_bins_ok(32) && dfl_bins_ok(64) && dfl_bins_ok(128) && dfl_bins_ok(256),
              "4·reg_max 的合法取值: 32/64/128/256 (reg_max = 8/16/32/64)");
        CHECK(!dfl_bins_ok(4) && !dfl_bins_ok(16),
              "4 与 16 (reg_max = 1/4) 拒绝: 桶太少, 且 4 是错类数最容易撞上的值");
        CHECK(!dfl_bins_ok(20) && !dfl_bins_ok(96) && !dfl_bins_ok(68),
              "20/96/68… 非 2 的幂拒绝 (20 = 4·5 正是'只查整除'会放过的那一类)");
        ...
        // 判据不误伤真配置: reg_max = 8 (4·reg_max = 32) 的最小合法形状照常解
        {
            ...
            std::vector<float> o8((size_t)g2 * g2 * a2, -1000.f);
            for (int i = 0; i < g2 * g2; ++i) {
                float* p = o8.data() + (size_t)i * a2;
                for (int k = 0; k < 4; ++k) p[k * reg8 + 0] = 0.f;   // 四条边都单点 bin 0
                p[4 * reg8] = 0.f;                                  // 类 0: logit 0 → 0.5
            }
            OutputLayout l8;
            parseOutputLayout(dims4(g2, g2, a2), l8, 640);          // 网格 4×4 → 步长 160
            CHECK(l8.dfl && l8.stride == 160 && dfl_bins_ok(l8.attrs - 1),
                  "reg_max = 8 (4·reg_max = 32) 是最小合法配置: 判据放行");
            auto d8 = decode_outputs({o8.data()}, {l8}, 1, 0.1f, -1, 640);
            CHECK(d8.size() == 16 && near_(d8[0].cx, 80) && near_(d8[0].cy, 80)
                  && near_(d8[0].w, 0) && near_(d8[0].h, 0),
                  "合法的 reg_max = 8 照常解码: 16 个格各一条, 距离全 0 → 框退化为格心 (80,80)");
        }
    }
