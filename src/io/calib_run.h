...
//  本的三个读数量的都是**物理环路延迟** (全都锚在帧时间戳上, 而推理整轮被跳过), 回写
//  下去的是它加推理段均值 —— 口径见文件末的 "回写" 段。
//
//  --- 观测模型 (一切读数的出发点) ---
...
//  写下去的值是**完整环路延迟** = 本引擎量的物理 L (三个读数都锚在帧时间戳上) + 推理段
//  均值 —— 标定整轮跳过推理, 推理那一腿只能由采集线程在正常运行时逐帧累计, 合成在调用方
//  一处完成 (io/capture.cpp 的标定段), 本引擎只负责把给的数落盘。冷启动 (还没跑过推理帧)
//  时没有均值可加, 只写物理值并在日志里说明。
// ============================================================================
...
//   三选一, 由当前输出模式定; 原子替换)。**写入的值不是本引擎的 l_est**: 引擎量到的是
//   **物理环路延迟** (三读数都锚在帧时间戳上), 而标定整轮跳过推理 —— 完整环路延迟 =
//   物理 L + 推理段均值 (那一腿由采集线程逐帧累计, 见 io/capture.cpp 的标定段), 合成在
//   调用方一处完成, 这里只负责把它落盘。r.ok 为假时什么都不写 (无编造的值)。
