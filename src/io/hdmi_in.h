...
//      (而不是把一张没写完的帧当成丢帧), 距 EDID 写完 0.9s (含 v4l2-ctl 自身与源的 HPD
//      反应时间)。
//    * 重建尝试 39 次, 间隔 2.1s (= 有界等锁 2000ms + 打开/复核 ~85ms), 每次如实报出
//      ("取源失败 (第 N 次): …"), 期间进程一直在跑 (USB 报告率 1000Hz 未断)。
//    * 锁回来的那一次 (部署 EDID 写回后 0.85s 源重锁, 内核 "hdmirx_wait_lock_and_get_timing
//      signal lock ok"): 重建本身耗时 **227ms**; 起流后驱动仍不产帧 (锁着但没有帧) → 2.0s
//      后判 Stalled 并再重建 (**16ms**) → 第一帧到手。**锁被写回到画面回来共 3.3s** (其中
//      2.0s 是那个 2000ms 的 poll 预算, 1.3s 是源自己的重锁)。
//    * 恢复之后 (余下 4.5 分钟): [AI FPS] 119–120fps, fence 超时 0 次, 失锁/停流/重建 0 次,
//      交付 40246 帧 / 驱动丢 3 / ERROR 0 —— 断流没有在流上留下残余。
//    * 内核日志侧: 等锁期间每 HDMI_LOCK_POLL_MS 问一次, 未锁时驱动每次自己打一行
//      hdmirx_query_dv_timings signal is not locked! (81s 窗口 433 行; 同一窗口驱动自己的
//      5s 延迟工作 16 行)。这是"有界等锁"的代价, 换来的是锁一回来就能在 200ms 内看见它。
...
//   实测落点 (板端 7 分钟含一次失锁, 40247 次等待): 成功的等待均值 4.98ms / 最长 7.80ms
//   (≈1 个 120fps 帧周期), 超时 **1 次** —— 恰好是失锁那一帧的载荷 (它的 fence 再没
//   signal), 白等 40ms。上限因此不动: 均值离 40ms 有 8 倍余量, 而 40ms 同时是 60fps 上
//   合法等待 (一帧 ≤16.7ms) 的 2.4 倍 —— 往小压只会把"慢但会写完"的帧判成丢帧。
