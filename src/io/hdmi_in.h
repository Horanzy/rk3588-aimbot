...
//        一帧是断流的第一个症状, 不必等下一帧。**一次 wait_frame 只问一次**: 答案不会因
//        为再问而改变 (失败过的查询只会继续失败), 而每次"未锁"的查询驱动都要自己打一行
//        内核日志 (实测代价见下方实测块)。
...
//
//  ---- 板端实测 (命令见 io/hdmi_in.cpp 头部; 2560×1440@120 活动信号, 源头是固定模式源) ----
//  给接收器写一份**别的** EDID 会让它落下 HPD。本次用的那份 (内核固件的 1920x1080.bin:
//  只有基块、没有 HDMI VSDB) 让源干脆不出图, 锁一直不回来 —— 于是"失锁 → 重建"的整条路
//  被完整走了一遍 (aimbot 连跑 420s, 失锁窗口 81s):
//    * 失锁被察觉在第一帧上: 那一帧的 dma-fence 没 signal 且锁查询失败 → 判 LockLost
//      (而不是把一张没写完的帧当成丢帧), 距 EDID 写完 0.1–0.9s —— 差异来自源的 HPD 反应
//      时间与 v4l2-ctl 自身的耗时, 本层那一侧只花了一帧。
//    * 重建尝试 39 次, 间隔 2.1s (= 有界等锁 2000ms + 打开/复核 ~85ms), 每次如实报出
//      ("取源失败 (第 N 次): …"), 期间进程一直在跑 (USB 报告率 1000Hz 未断)。
//    * 锁回来的那一次 (部署 EDID 写回后源重锁): 重建本身耗时 **227ms**; 起流后驱动仍不产帧
//      (锁着但没有帧) → 2.0s 后判 Stalled 并再重建 (**24ms**) → 第一帧到手。**锁被写回到
//      画面回来 3.3–4.0s** (两次实测), 其中 2.0s 是那个 2000ms 的 poll 预算, 其余是源自己
//      的重锁与两次重建。
//    * 恢复之后: [AI FPS] 119–120fps, fence 超时 0 次, 失锁/停流/重建 0 次, 交付 40232 帧 /
//      取代 2 / 驱动丢 0 / ERROR 0 —— 断流没有在流上留下残余。
//    * 内核日志侧: 等锁期间每 HDMI_LOCK_POLL_MS 问一次, 未锁时驱动每次自己打一行
//      hdmirx_query_dv_timings signal is not locked! (81s 窗口 467 行 ≈ 5.8/s; 同一窗口驱动
//      自己的 5s 延迟工作 4 行)。这是"有界等锁"的代价, 换来的是锁一回来就能在 200ms 内看见它。
//    * 驱动对那一次丢帧的兜底也在同一个窗口里: hdmirx_dqbuf_get_done_fence: last fence
//      not signal, signal now! (HDMI_FENCE_FAIL_MAX 注释里那句内核行为的现场证据)。
//  这台源是固定模式源 (换 EDID 的首选模式、带宽声明、模式表都不改它的输出, 只有一份连
//  HDMI 能力都没有的 EDID 才让它停), 故两次重建读到的格式相同; 分辨率/帧率变化走的是
//  同一段代码 —— 每次重建都在锁之后读 G_FMT (见 arm), 与源无关。
//
...
//   实测落点 (板端 420s 连跑含一次 81s 失锁, 交付 40232 帧, 等 fence 40235 次): 成功的
//   等待均值 5.01ms / 最长 8.31ms (≈1 个 120fps 帧周期), 超时 **3 次** —— 白等 120ms,
//   占运行时长 0.03%, 其中一次正是失锁那一帧的载荷 (它的 fence 再没 signal)。上限因此
//   不动: 均值离 40ms 有 8 倍余量, 而 40ms 同时是 60fps 上合法等待 (一帧 ≤16.7ms) 的
//   2.4 倍 —— 往小压只会把"慢但会写完"的帧判成丢帧。
...
//   事重复问一遍。实测: 失锁窗口里 39 次尝试的实际间隔 2.1s (有界等锁把节拍顶住), 而锁
//   回来的那一次重建只用 227ms —— 节拍从不是恢复时间的那一项。
