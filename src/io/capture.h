...
//  **回写与运行态生效的都是标定量的物理环路延迟**: 三个读数都锚在帧时间戳上, 而律的时间戳
//  锚点是**帧交付时刻** (wait_frame 返回处取的 now) —— 该时刻之后的 RGA 与推理处理 (H2D/
//  exec/D2H/解码, 即 [AI FPS] 行报的那一段) 由律的 age 与 gap_scale 在每一拍里承载, 再加进
//  L 就是把同一段算两遍 (实测 640 模型: 推理段 6.39ms, 信念从 50ms 抬到 56.4ms 时 arena 匹配档
//  composite 114.66 → 129.11 — 复现命令 arena.eval ff_pi_acc 56.4)。推理段均值仍在
//  [AI FPS] 行里报, 它是诊断量, 不参与计算。
//  **取帧与 RGA 同理不加**: 它们是两边共付的腿 (标定那轮自己也要等帧、也要裁一次 640 窗口),
//  加进去同样是算两遍。
...
//                                               与**推理段的累计均值** (诊断: 律的锚点是帧交付
//                                               时刻, 其后的处理由 age 承载, 不进 L)
