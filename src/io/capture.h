...
//  **回写的是完整环路延迟** = 标定量的物理 L + 推理段均值 (打包/H2D/exec/D2H/解码, 即
//  [AI FPS] 行报的那一段): 标定整轮跳过推理, 于是这一腿不在它的三个读数里, 而律看到的
//  帧确实过了这一腿 —— 由本线程逐帧累计 (只计走完整条链的帧), 在回写处一次合成, 运行态
//  的 l_est 与脚本里那个数因此是同一个 (冷启动没有均值时只写物理值并在日志里说明)。
//  **取帧与 RGA 不加**: 它们是两边共付的腿 (标定那轮自己也要等帧、也要裁一次 640 窗口),
//  加进去就是把已经量过的延迟再算一遍。
