...
// AI 采集/推理线程 (main 孵化): 打开接收器 (io/hdmi_in, 缺省按驱动名解析) 与模型,
//   逐帧跑上面那条链并把目标发布给控制拍 (estimator_step), 同时承担标定采样/拟合/回写
//   与三源截图、预览。采集或模型任一打不开即结束整个进程 (取不到画面就没有目标, 而
//   "正在运行"是一个比"报错退出"更糟的状态)。
