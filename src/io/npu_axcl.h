...
//    斜率加各尺寸自己的实测值, 不拿直线外推)。H2D 的源就是调用方交进来的那块缓冲本身
//    (采集侧给的是 RGA 目的缓冲的映射, 它的缓存一致性由采集层负责, 见 io/rga_pp.h): 一次
//    传输就是一次传输, 中间不再有纯搬运的打包拷贝。这条捷径的代价是"源的字节数必须与
//    边长²×3 相符"从"打包保证"变成"调用方保证", 故该契约在 open 时当场核对 (不符即打不开,
//    不会越读), 而失效化的责任也随缓冲一起落在采集层 —— 由采集层负责的那一份约定不变。
//
